...
interface AdminOrderRowProps {
  order: Order;
  handleUpdateStatus: (id: string, status: Order['status'], tracking: string) => any;
  handleDeleteOrder: (id: string) => any;
}

const AdminOrderRow: React.FC<AdminOrderRowProps> = ({ order, handleUpdateStatus, handleDeleteOrder }) => {
  ...
  const statusColors = {
    ...
    shipped: 'bg-indigo-50 text-indigo-700 border-indigo-250',
    delivered: 'bg-emerald-50 text-emerald-700 border-emerald-250'
  };

  const isSaveDisabled = status === order.status && tracking === (order.trackingNumber || '');

  return (
    <div className="border border-gray-150 rounded-xl overflow-hidden bg-white hover:shadow-xs transition-shadow">
      {/* Header row */}
      <div 
        onClick={() => setIsExpanded(!isExpanded)}
        className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer hover:bg-gray-50/50 transition-colors select-none"
      >
        <div className="space-y-1.5 text-left">
          <div className="flex items-center gap-2 flex-wrap text-sm">
            <span className="font-mono font-black text-xs text-[#DE5246] tracking-tight">{order.id}</span>
            <span className="text-gray-300 text-xs">•</span>
            <span className="text-xs font-semibold text-gray-500 font-mono flex items-center gap-1">
              <Calendar className="w-3.5 h-3.5 text-gray-400" /> {order.orderDate}
            </span>
            <span className="text-gray-300 text-xs">•</span>
            <span className="text-xs font-bold text-gray-800 font-sans flex items-center gap-1">
              <User className="w-3.5 h-3.5 text-gray-400" /> {order.userName}
            </span>
          </div>
          
          <div className="flex gap-1.5 flex-wrap">
            {order.items.map((it, idx) => (
              <span key={idx} className="bg-gray-50 text-gray-700 text-[10px] font-sans px-2 py-0.5 rounded border border-gray-200/60">
                {it.name} <strong className="text-[#DE5246]">x{it.quantity}</strong>
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 self-start sm:self-center">
          ...
          <span className={`text-[9px] border px-2.5 py-0.5 rounded-full font-mono font-bold uppercase tracking-wider ${statusColors[order.status] || 'bg-gray-50'}`}>
            {order.status}
          </span>
          <ChevronRight className={`w-4 h-4 text-gray-450 transition-transform duration-200 ${isExpanded ? 'rotate-90 text-gray-700' : ''}`} />
        </div>
      </div>

      {/* Expanded section */}
      {isExpanded && (
        <div className="border-t border-gray-100 bg-[#FAF9F5]/45 p-4 sm:p-6 space-y-6 text-left font-sans animate-fadeIn">
          {/* Bento client info card grid */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            
            {/* Box 1: Client Contacts */}
            <div className="bg-white rounded-xl border border-gray-150 p-4 space-y-3 shadow-3xs">
              <div className="flex items-center gap-1.5 pb-2 border-b border-gray-100">
                <User className="w-3.5 h-3.5 text-[#DE5246]" />
                <h4 className="text-[10px] font-black uppercase text-slate-900 tracking-wider">Client Credentials</h4>
              </div>
              <ul className="space-y-2 text-xs leading-relaxed text-gray-650">
                <li>
                  <span className="text-gray-400 text-[8px] uppercase font-bold block tracking-wider">Purchaser Name</span>
                  <strong className="text-gray-900 text-xs font-sans">{order.userName}</strong>
                </li>
                <li>
                  <span className="text-gray-400 text-[8px] uppercase font-bold block tracking-wider">Direct Mailbox</span>
                  <a href={`mailto:${order.userEmail}`} className="text-[#DE5246] hover:underline flex items-center gap-1 font-mono text-[11px] break-all">
                    <Mail className="w-3 h-3 text-gray-400 shrink-0" />
                    {order.userEmail}
                  </a>
                </li>
                <li>
                  <span className="text-gray-400 text-[8px] uppercase font-bold block tracking-wider">Contact Line</span>
                  <span className="text-gray-900 flex items-center gap-1 font-mono text-[11px]">
                    <Phone className="w-3 h-3 text-gray-400 shrink-0" />
                    {order.phone || 'None provided'}
                  </span>
                </li>
              </ul>
            </div>

            {/* Box 2: Physical Delivery Coordinates */}
            <div className="bg-white rounded-xl border border-gray-150 p-4 space-y-3 shadow-3xs">
              <div className="flex items-center gap-1.5 pb-2 border-b border-gray-100">
                <MapPin className="w-3.5 h-3.5 text-[#DE5246]" />
                <h4 className="text-[10px] font-black uppercase text-slate-900 tracking-wider">Delivery Address</h4>
              </div>
              <div className="text-xs text-gray-650 leading-relaxed space-y-1">
                <span className="text-gray-400 text-[8px] uppercase font-bold block tracking-wider">Physical Coordinates</span>
                <p className="font-bold text-slate-900">{order.shippingAddress?.street}</p>
                <p>{order.shippingAddress?.city}, {order.shippingAddress?.state} {order.shippingAddress?.postalCode}</p>
                <p className="text-[9px] uppercase font-bold tracking-wider text-gray-500 font-mono">{order.shippingAddress?.country}</p>
              </div>
            </div>

            {/* Box 3: Payment Logs & Notes */}
            <div className="bg-white rounded-xl border border-gray-150 p-4 space-y-3 shadow-3xs">
              <div className="flex items-center gap-1.5 pb-2 border-b border-gray-100">
                <DollarSign className="w-3.5 h-3.5 text-[#DE5246]" />
                <h4 className="text-[10px] font-black uppercase text-slate-900 tracking-wider">Acquisition Diagnostics</h4>
              </div>
              <ul className="space-y-2 text-xs text-gray-650 leading-relaxed">
                <li>
                  <span className="text-gray-400 text-[8px] uppercase font-bold block tracking-wider">Settlement Route</span>
                  <span className="bg-gray-100 text-slate-850 px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase border border-gray-200 inline-block mt-0.5">
                    {order.paymentMethod === 'crypto' ? '✅ USDT / TRC20 Crypto' : order.paymentMethod === 'bank_transfer' ? '✅ PAYID / Direct' : '✅ Credit Card'}
                  </span>
                </li>
                <li>
                  <span className="text-gray-400 text-[8px] uppercase font-bold block tracking-wider">Diagnostics / Reference Logs</span>
                  <p className="font-mono text-[10px] bg-slate-50 border border-slate-100 p-1.5 rounded text-gray-750 whitespace-pre-wrap leading-tight max-h-[60px] overflow-y-auto">
                    {order.paymentDetails || 'No auxiliary info logged.'}
                  </p>
                </li>
              </ul>
            </div>
          ...
          {/* Research Note if provided */}
          {order.note && (
            <div className="p-3.5 bg-rose-50/50 border border-dashed border-rose-150 rounded-xl text-xs text-rose-950">
              <div className="font-bold flex items-center gap-1.5 text-rose-800 uppercase text-[9px] tracking-wider mb-1">
                <FileText className="w-3.5 h-3.5" /> Researcher Special Notes Instructions:
              </div>
              <p className="italic font-sans leading-relaxed">"{order.note}"</p>
            </div>
          )}

          {/* Reagents list detailed table */}
          <div className="bg-white rounded-xl border border-gray-150 overflow-hidden shadow-3xs">
            <div className="px-4 py-2 border-b border-gray-100 bg-[#FAFBFD] text-[9px] uppercase font-bold font-mono tracking-wider text-gray-400">
              Assay Product Ledger Details
            </div>
            <div className="p-3 divide-y divide-gray-100 font-sans text-xs">
              {order.items.map((it, idx) => (
                <div key={idx} className="py-2 flex justify-between items-center flex-wrap gap-2 text-xs">
                  <div className="text-left">
                    <p className="font-bold text-slate-900">{it.name}</p>
                    <p className="text-[10px] text-gray-400 font-mono">Product Reference: {it.productId}</p>
                  </div>
                  <div className="font-mono text-xs text-gray-700">
                    <span>x{it.quantity}</span>
                    <span className="mx-2 text-gray-300">•</span>
                    <span>${Number(it.priceAtPurchase).toFixed(2)}</span>
                    <span className="mx-2 text-gray-300">•</span>
                    <span className="font-bold text-slate-900">${(Number(it.priceAtPurchase) * Number(it.quantity)).toFixed(2)} USD</span>
                  </div>
                </div>
              ))}
              <div className="pt-2.5 mt-1 flex justify-between items-center font-bold text-slate-900 text-xs">
                <span>Total Reagents Cost:</span>
                <span className="text-sm font-sans text-[#DE5246] font-extrabold font-mono">${order.total.toFixed(2)} USD</span>
              </div>
            </div>
          </div>

          {/* Update controls block */}
          <div className="flex flex-col lg:flex-row items-stretch lg:items-end justify-between gap-6 pt-4 border-t border-gray-100/80">
            <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-end w-full lg:max-w-2xl">
              
              {/* Status input */}
              <div className="space-y-1 text-left w-full sm:w-1/3">
                <label className="text-gray-500 text-[8px] uppercase font-bold tracking-wider block">Assigned Order Status</label>
                <select
                  value={status}
                  onChange={(e) => setStatus(e.target.value as any)}
                  className="w-full bg-white border border-gray-200 hover:border-gray-300 p-2 rounded-xl text-xs font-semibold text-slate-800 outline-none transition-colors cursor-pointer"
                >
                  <option value="pending">🟡 Pending (Unpaid)</option>
                  <option value="processing">🔵 Processing (Paid)</option>
                  <option value="shipped">🟣 Shipped (Transit)</option>
                  <option value="delivered">🟢 Delivered (Fulfill)</option>
                </select>
              </div>

              {/* Tracking input */}
              <div className="space-y-1 text-left w-full sm:w-2/3">
                <label className="text-gray-500 text-[8px] uppercase font-bold tracking-wider block">Courier Dispatch Tracking Number</label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={tracking}
                    onChange={(e) => setTracking(e.target.value)}
                    className="w-full bg-white border border-gray-200 hover:border-gray-300 p-2 rounded-xl text-xs font-mono text-slate-850 outline-none transition-colors placeholder:text-gray-300"
                    placeholder="e.g. AUS-POST-992140"
                  />
                  <button
                    onClick={() => handleUpdateStatus(order.id, status, tracking)}
                    disabled={isSaveDisabled}
                    className={`px-4 py-2 rounded-xl font-bold font-mono text-[9px] uppercase tracking-wider transition-all shadow-3xs cursor-pointer ${
                      isSaveDisabled 
                        ? 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200' 
                        : 'bg-black text-white hover:bg-stone-900 border border-black'
                    }`}
                  >
                    Commit
                  </button>
            ...
            <div className="flex flex-col items-stretch md:items-end gap-1.5 shrink-0">
              <span className="text-[8px] text-gray-400 font-mono tracking-wider block">REGISTRY AUDIT CLEARANCE:</span>
              {order.status === 'delivered' ? (
                <button
                  onClick={() => handleDeleteOrder(order.id)}
                  className="px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 hover:text-rose-850 border border-rose-200 hover:border-rose-300 rounded-xl text-[10px] font-mono font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-3xs"
                  title="Purge successfully delivered order from active list"
                >
                ...
                <div className="flex flex-col lg:items-end gap-1 text-left lg:text-right">
                  <button
                    disabled
                    className="px-4 py-2 bg-gray-50 text-gray-400 border border-gray-150 rounded-xl text-[10px] font-mono font-bold uppercase tracking-wider cursor-not-allowed flex items-center gap-1.5 opacity-60 w-full lg:w-auto justify-center"
                    title="Orders must be delivered before registry purging is permitted"
                  >
                    <Lock className="w-3.5 h-3.5 text-gray-300" />
                    Delete Order
                  </button>
                  <span className="text-[9px] text-amber-600 font-mono tracking-tight font-medium">
                    ⚠️ Set status to Delivered to unlock deletion
                  </span>
                </div>
              )}
            </div>
...
interface AdminQueryRowProps {
  query: any;
  handleUpdateStatus: (id: string, status: string) => void;
  handleDelete: (id: string) => void;
}

const AdminQueryRow: React.FC<AdminQueryRowProps> = ({ query, handleUpdateStatus, handleDelete }) => {
  const [isExpanded, setIsExpanded] = useState(false);

  return (
    <div className="bg-white border border-gray-150 rounded-xl p-4 transition-all hover:border-[#DE5246]/30 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-bold text-gray-900 text-[13px]">{query.name}</span>
            <span className="text-[10px] text-gray-400 font-mono">({query.email})</span>
          ...
          <div className="text-[11px] text-gray-700 font-semibold">{query.subject}</div>
          <div className="text-[10px] text-gray-400 font-mono">{query.date}</div>
        </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-center shrink-0">
          <select
            ...
            className="text-[11px] font-bold py-1 px-2.5 rounded-lg border border-gray-200 bg-gray-50 focus:outline-none focus:ring-1 focus:ring-[#DE5246]"
          >
          ...
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="px-3 py-1 bg-gray-900 hover:bg-black text-white text-[11px] font-bold rounded-lg cursor-pointer transition-all"
          >
          ...
          <button
            ...
            className="p-1 px-2 text-rose-600 hover:text-rose-800 hover:bg-rose-50 rounded-lg transition-colors border border-transparent hover:border-rose-100 cursor-pointer"
      ...
      {isExpanded && (
        <div className="mt-3 pt-3 border-t border-gray-100 text-[12px] text-gray-700 leading-relaxed bg-[#FAF9F5]/40 p-3 rounded-lg border border-dashed border-gray-150">
          <p className="font-bold text-[#0C1B2D] text-[10px] uppercase tracking-wider mb-1">Message Body:</p>
          <div className="whitespace-pre-wrap font-sans text-gray-750">{query.message}</div>
          <div className="mt-3 flex justify-end gap-2">
            <a
              href={`mailto:${query.email}?subject=Re: ${encodeURIComponent(query.subject)}`}
              className="px-3 py-1 bg-[#DE5246] hover:bg-[#c94539] text-white rounded text-[11px] font-bold transition-all cursor-pointer inline-flex items-center gap-1 font-mono uppercase tracking-wider"
            >
              ✉ Reply Email
            </a>
          </div>
...
export default function AdminPortal({
  ...
  // 0. CONTACT QUERIES & SUB-TABS STATE
  const [queries, setQueries] = useState<any[]>([]);
  const [loadingQueries, setLoadingQueries] = useState(false);
  const [queriesError, setQueriesError] = useState('');
  const [subTab, setSubTab] = useState<'orders' | 'queries'>('orders');
  ...
  const fetchQueries = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setQueriesError(data.error || 'Failed to retrieve queries');
      }
    } catch (err: any) {
      setQueriesError(err.toString());
    } finally {
  ...
  const handleUpdateQueryStatus = async (id: string, newStatus: string) => {
    try {
      ...
      console.error("Error updating query status on server:", err);
    }
  };
  ...
  const handleDeleteQuery = async (id: string) => {
    ...
    try {
      ...
      console.error("Error deleting contact query on server:", err);
    }
  };
  ...
  // EmailJS Integration configurations
  const [emailProvider, setEmailProvider] = useState<'smtp' | 'emailjs'>(() => {
    return (localStorage.getItem('peps_email_provider') as 'smtp' | 'emailjs') || 'smtp';
  });
  const [emailjsServiceId, setEmailjsServiceId] = useState(() => {
    return localStorage.getItem('peps_emailjs_service_id') || '';
  });
  const [emailjsOrderTemplateId, setEmailjsOrderTemplateId] = useState(() => {
    return localStorage.getItem('peps_emailjs_order_template_id') || '';
  });
  const [emailjsContactTemplateId, setEmailjsContactTemplateId] = useState(() => {
    return localStorage.getItem('peps_emailjs_contact_template_id') || '';
  });
  const [emailjsPublicKey, setEmailjsPublicKey] = useState(() => {
    return localStorage.getItem('peps_emailjs_public_key') || '';
  });
...
Once reviewed, approved refunds will be credited back via PAYID or the original payment method within 2-3 business days. Your satisfaction and scientific confidence in your reagents constitutes our absolute highest operational priority.`;
  ...
  const handleSaveEmailConfig = (e: React.FormEvent) => {
    e.preventDefault();
    localStorage.setItem('peps_email_provider', emailProvider);
    localStorage.setItem('peps_emailjs_service_id', emailjsServiceId.trim());
    localStorage.setItem('peps_emailjs_order_template_id', emailjsOrderTemplateId.trim());
    localStorage.setItem('peps_emailjs_contact_template_id', emailjsContactTemplateId.trim());
    localStorage.setItem('peps_emailjs_public_key', emailjsPublicKey.trim());
    ...
    setChannelSuccessMsg("Email delivery configurations successfully saved in real-time!");
    setTimeout(() => setChannelSuccessMsg(""), 4000);
  };
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      const response = await fetch('/api/send-test-email', {
        ...
        body: JSON.stringify({
          ...
          emailjsConfig: {
            ...
            orderTemplateId: emailjsOrderTemplateId.trim(),
            contactTemplateId: emailjsContactTemplateId.trim(),
      ...
      if (response.ok && data.success) {
        ...
        setSmtpTestResult({
          success: false,
          error: data.error || 'Server rejected email dispatch test.',
          helper: data.helper
        });
      }
    } catch (err: any) {
      setSmtpTestResult({
        success: false,
        error: err.toString() || 'Connection failed to reach email dispatch server.'
      });
    } finally {
  ...
  const handleUpdateOrderStatus = async (orderId: string, newStatus: Order['status'], tracking: string) => {
    try {
      ...
      if (data.success) {
        const updated = orders.map(o => {
          if (o.id === orderId) {
            return { ...o, status: newStatus, trackingNumber: tracking };
          }
          return o;
        });
        setOrders(updated);
        ...
        alert("Failed to update order status on server: " + (data.error || "Unknown response error."));
      }
    } catch (err: any) {
      console.error(err);
      alert("Network or server connection failed. Syncing local backup registry state.");
      const updated = orders.map(o => {
        if (o.id === orderId) {
          return { ...o, status: newStatus, trackingNumber: tracking };
        }
  ...
  const handleDeleteOrder = async (orderId: string) => {
    if (!window.confirm(`PERMANENT DELETION INSTRUCTION RECEIVED:\nAre you sure you want to delete order ${orderId} from the central registry?`)) {
      return;
    }
    try {
      const response = await fetch(`/api/orders/${orderId}`, {
      ...
      if (data.success) {
        ...
        setOrders(updated);
        localStorage.setItem('peps_au_orders', JSON.stringify(updated));
        alert(`Order ${orderId} successfully removed from the central database.`);
      } else {
        alert("Server rejected the delete command: " + (data.error || "Unknown response."));
      }
    } catch (err: any) {
      console.error(err);
      alert("Server is offline or unreachable. purging from local registry state.");
      const updated = orders.filter(o => o.id !== orderId);
      setOrders(updated);
      localStorage.setItem('peps_au_orders', JSON.stringify(updated));
    }
          ...
          <div className="space-y-6 font-sans text-xs text-left">
            ...
            <div className="flex border-b border-gray-150 gap-2">
              <button
                ...
                className={`py-2.5 px-5 text-xs font-bold uppercase tracking-wider transition-all border-b-2 flex items-center gap-1.5 cursor-pointer ${
                  subTab === 'orders'
                    ? 'border-[#DE5246] text-[#DE5246]'
                    : 'border-transparent text-gray-500 hover:text-gray-900'
                }`}
              >
                📦 Placed Orders Directory ({filteredOrdersList.length})
              </button>
              <button
                type="button"
              ...
              >
                💬 Contact Inquiries Ledger ({queries.length})
              </button>
            </div>
            ...
            {subTab === 'orders' ? (
              <div className="bg-white rounded-2xl border border-gray-150 shadow-3xs overflow-hidden text-left">
                ...
                <div className="p-4 space-y-4">
                  {filteredOrdersList.length === 0 ? (
                    <div className="p-12 text-center text-gray-400 font-mono bg-gray-50 rounded-xl border border-dashed border-gray-150">
                    ...
                    filteredOrdersList.map(o => (
                      <AdminOrderRow
                        key={o.id}
                        order={o}
                        handleUpdateStatus={handleUpdateOrderStatus}
              ...
              <div className="bg-white rounded-2xl border border-gray-150 shadow-3xs overflow-hidden text-left">
                <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center text-xs font-mono font-bold text-gray-500">
                  <span className="uppercase tracking-wider">💬 Visitor Queries Ledger</span>
                  <span className="text-xs bg-[#DE5246]/10 text-[#DE5246] px-2.5 py-0.5 rounded-full font-sans font-bold">
                    {queries.length} total inquiries
                  </span>
                </div>

                <div className="p-4 space-y-4">
                  {loadingQueries ? (
                    <div className="p-12 text-center text-gray-500 font-mono bg-gray-50 rounded-xl border border-dashed border-gray-150 animate-pulse">
                      Retrieving active research queries from dynamic server file registry...
                    </div>
                  ) : queriesError ? (
                    <div className="p-12 text-center text-rose-500 font-sans bg-rose-50 rounded-xl border border-rose-150 font-medium">
                      Error fetching inquiries: {queriesError}
                    </div>
                  ) : queries.length === 0 ? (
                    <div className="p-12 text-center text-gray-400 font-mono bg-gray-50 rounded-xl border border-dashed border-gray-150">
                      No customer contact inquiries found in the registry.
                    </div>
                  ) : (
                    queries.map(q => (
                      <AdminQueryRow
                        key={q.id}
                        query={q}
                        handleUpdateStatus={handleUpdateQueryStatus}
